...
const ColorPaletteModal = ({navigation}) =>{
  const [CustomColorArr,setCustomColorArr]=useState([])
  ...
  const OnSubmit = useCallback(()=>{
    const NewPalette={
      ...
      colors:CustomColorArr,
    }
    if (!CustomPaletteName || NewPalette.colors.length < 3 ) {
      if (!CustomPaletteName) {
        return Alert.alert("Error",'Please Enter Palette Name')
      }else{
        return Alert.alert("Error",'Please select more than 3 colors')
      }
    }else{
  ...
  },[CustomPaletteName,CustomColorArr])

  const AddingColorInArray = useCallback((value,ColorObject)=>{
    if (value===true) {
      setCustomColorArr(colors=>[...colors,ColorObject])
    }else{
      setCustomColorArr(colors=>colors.filter(color=>color.colorName!==ColorObject.colorName))
    }
  },[])
  ...
  return(
    <View style={styles.container}>
          <Text style={styles.listText}>Name of the color palette</Text>
          <TextInput
            style={styles.input}
          ...
          renderItem={({item,index})=>(
            <View>
                     <View style={styles.colorList}>
                            ...
                            <Switch
                            value={!!CustomColorArr.find(color=>color.colorName=== item.colorName)}
                            onValueChange={selected=>AddingColorInArray(selected,item)}
                            />
